...
class MusicPlayerApp:
    ...
    def setup_content_area(self, parent):
        ...
        sort_by = ttk.Combobox(sort_frame, values=['title', 'artist', 'year'], 
    ...
    def setup_player_controls(self):
        ...
        # Current song info
        self.current_song_label = tk.Label(player, text="Tidak ada lagu diputar", 
                                          font=('Arial', 12, 'bold'), 
                                          bg=self.colors['bg_sec'], 
                                          fg=self.colors['text'])
        self.current_song_label.pack(side='left', padx=30, pady=10)
    ...
    def show_context_menu(self, event):
        ...
        if item:
            ...
            # Menu favorite
            if song:
                fav_text = "💔 Hapus dari Favorite" if song.get('favorite', False) else "❤️ Tambah ke Favorite"
                self.context_menu.add_separator()
    ...
    def show_library(self):
        ...
        self.current_view = 'library'
        self.selected_playlist = None
        self.title_label.config(text="Library Musik")
        self.refresh_song_list()
    ...
    def update_player_ui(self):
        ...
        self.current_song_label.config(
            text=f"🎵 {self.current_song['title']} - {self.current_song['artist']}"
        )
        
        if self.is_playing:
            self.status_label.config(text="▶ Sedang Diputar")
